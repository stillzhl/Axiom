...
- [ ] T1: Implement `axiom.store` (SQLite adapter): ledger open/close, WAL
...
- [ ] T2: Implement `axiom.ledger` (pure port): event validation via the
...
- [ ] T3: Snapshot replay equivalence: `restore(snapshot at N) +
  ...
  deliveries).
- [ ] T4: Implement `axiom.cli replay` (`--ledger PATH [--through SEQ]`,
...
- [ ] T5: Implement `axiom.cli export-bundle` (`--ledger PATH --output PATH
...
- [ ] T6: Schema migration: `schema_version` table, numbered forward-only
...
- [ ] T7: Crash and adversarial tests: interrupted append leaves no phantom
