...
- [ ] Git observation of a synthetic repo reproduces identical
...
- [ ] Path safety: symlink-escape cases are classified `:path/unsafe`
  ...
  changes and Unicode paths are enumerated exactly.
- [ ] Artifact digests match `sha256sum` on the same bytes; retention
  ...
  equivalence.
- [ ] Runner timeout, cancellation and output-cap produce `:incomplete`
...
- [ ] Observation/evidence events append through the 0002 path and replay
...
Spec status: **Accepted** (implementation pending). No milestone
